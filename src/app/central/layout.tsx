...
import { AccountBalanceOutlined, AnalyticsOutlined, DashboardOutlined, HistoryOutlined, MapOutlined, PersonOutlined, ReceiptLongOutlined } from "@mui/icons-material";
...
const CENTRAL_NAV: AdminNavSection[] = [
  {
    ...
    label: "Main",
    items: [
      { id: "/central/dashboard", label: "Dashboard", icon: <DashboardOutlined sx={{ fontSize: 20 }} /> },
      { id: "/central/analytics", label: "Analytics", icon: <AnalyticsOutlined sx={{ fontSize: 20 }} /> },
      { id: "/central/national-map", label: "National Map", icon: <MapOutlined sx={{ fontSize: 20 }} /> },
    ],
  },
  {
    type: "group",
    label: "Management",
    items: [
      { id: "/central/province-admins", label: "Province Admins", icon: <PersonOutlined sx={{ fontSize: 20 }} /> },
      { id: "/central/eligibility-rules", label: "Eligibility Rules", icon: <ReceiptLongOutlined sx={{ fontSize: 20 }} /> },
      { id: "/central/audit-log", label: "Audit Log", icon: <HistoryOutlined sx={{ fontSize: 20 }} /> },
      { id: "/central/policy-cards", label: "Policy Cards", icon: <ReceiptLongOutlined sx={{ fontSize: 20 }} /> },
    ],
  },
];
...
function getActiveView(pathname: string) {
  ...
  if (pathname.startsWith("/central/national-map")) return "/central/national-map";
  if (pathname.startsWith("/central/province-admins")) return "/central/province-admins";
  if (pathname.startsWith("/central/eligibility-rules")) return "/central/eligibility-rules";
  if (pathname.startsWith("/central/audit-log")) return "/central/audit-log";
  if (pathname.startsWith("/central/policy-cards")) return "/central/policy-cards";
...
export default function CentralLayout({ children }: { children: React.ReactNode }) {
  ...
  const activeLabel = getNavItems(CENTRAL_NAV).find((item) => item.id === activeItem)?.label ?? "Dashboard";
