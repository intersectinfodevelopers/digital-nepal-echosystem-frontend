    ...
    <div className="pb-6">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-500">{filtered.length} citizen{filtered.length === 1 ? "" : "s"} registered in this ward</p>
        </div>
        <Link
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-gray-100 bg-gray-50">
                <th className="text-left px-4 py-3 font-medium text-gray-600">Name (NP)</th>
                <th className="text-left px-4 py-3 font-medium text-gray-600">Name (EN)</th>
                <th className="text-left px-4 py-3 font-medium text-gray-600">NID / Citizenship</th>
            ...
            <tbody>
              {paginated.map((c) => {
                ...
                const created = cr.created_at ? String(cr.created_at).slice(0, 10) : "";
                const sex = String(c.sex ?? "");
                return (
                  <tr
                    key={c.id ?? `${c.name_en}-${c.nid_masked}`}
                    className="border-b border-gray-100 hover:bg-gray-50"
                  >
                    <td className="px-4 py-3 text-gray-900">{c.name_np || "—"}</td>
                    <td className="px-4 py-3 text-gray-900">{c.name_en || "—"}</td>
                    <td className="px-4 py-3 text-gray-600 font-mono">{c.nid_masked || "—"}</td>
                    <td className="px-4 py-3 text-gray-600">{sex ? sex.charAt(0) + sex.slice(1).toLowerCase() : "—"}</td>
                    <td className="px-4 py-3 text-gray-600">{c.employment_category || "—"}</td>
